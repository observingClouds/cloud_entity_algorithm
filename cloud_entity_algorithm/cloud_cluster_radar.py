...
def get_args():
    ...
    parser.add_argument('-i', '--inputfilefmt',
    ...
    parser.add_argument('-o', '--outputfilefmt',
    ...
    parser.add_argument('-d', '--date', metavar="YYYYMM",
                        help='Provide the desired month, Format: YYYYMM',
                        required=True, default=None)

    parser.add_argument('-s', '--cloud-stencil', metavar='1 10',
                        help='Stencil connecting cloud pixels'
                        '(height, time)', required=False,
                        default=(1, 10), nargs=2, type=int)

    parser.add_argument('-t', '--cloud-threshold', metavar='-55',
                        help='Reflectivity threshold (dBZ) for clouds. Lower'
                        'values are ignored during labeling', required=False,
                        default=-55, nargs=1, type=int)

    parser.add_argument('-r', '--max-range', metavar='150',
                        help='Highest rangegate of radar that should'
                        'still be included in analysis', required=False,
                        default=150, nargs=1, type=int)
...
try:
    git_module_version = subprocess.check_output(["git", "describe", "--dirty"]).strip().decode() 
except:
    git_module_version = "--"
...
filename_radar_monotonic = outputfmt.format(level='Z_gridded', date=date_YYYYMM)  # f'MBR2__BCO__Zf__10s_monotonic_grid__201801.nc'
filename_label = outputfmt.format(level='CloudEntity', date=date_YYYYMM)  # MBR2__BCO__Labels__10s_monotonic_grid__201801.nc'
filename_entity = outputfmt.format(level='CloudEntityInformation', date=date_YYYYMM)  # 'MBR2__BCO__CloudEntityInformations__201801_.nc'

if RESAMPLE:
    logging.info('Resampling data')
    # Gather files
    input_radar_files = sorted(glob.glob(input_args["inputfilefmt"].format(date=date_YYMM)))# data_dir+"MMCR__MBR__Spectral_Moments__10s__155m-*km__18010?.nc"))
    ...
    idx_2_del = []
    for time in ['180413', '150608', '150727', '150728', '150811', '151026',
                 '160318', '170309', '170615', '150730', '180108']:
        try:
    ...
    logging.info('Non unique timestamps: {}'.format(len(Z.time)-len(np.unique(Z.time))))
    ...
    ds.attrs['title'] = 'Radar reflectivity on equidistant monotonic'\
                       ' increasing time grid'
    ds.attrs['source_files_used'] = np.array(input_radar_files)[idx_sort]
    ds.attrs['creation_date'] = datetime.datetime.utcnow().strftime("%d/%m/%Y %H:%M UTC")
    ds.attrs['created_with'] = os.path.basename(__file__) + " with its last modification on " + ctime(
...
if LABELING:
    ...
    labels_original = labels*np.asarray(data, dtype=bool)
    labels_original = np.where(labels_original == 0, np.nan,
                               labels_original)  # remove zeros which are created by the 0-cloud-labels
    ...
    var_h.units = 'm'
    var_label = nc.createVariable("label", np.int, ('range', 'time'),
                                  zlib=True)
    var_label.stencil = str(stencil_label.shape)
    var_label.reflectiviy_threshold = cloud_threshold
    var_label.description = 'Cloud entities created by applying a stencil'\
    ...
    nc.description = 'Cloud labels derived from radar reflectivity smoothed'\
    ...
    nc.created_with = os.path.basename(__file__) + " with its last modification on " + ctime(
                os.path.getmtime(os.path.realpath(__file__)))
...
if ANALYSIS:
    ...
    vf_apply_cloudtype = np.vectorize(ccam.f_apply_cloudtype,
                                      otypes=[np.float])
    ...
    def get_stsc_macrophysics(StSc_idx, cbhs, cths):
        ...
        return np.min(thicknesses), np.max(thicknesses),\
            np.mean(thicknesses), np.std(thicknesses)
    ...
    properties_str = ["cloud", "entity_start_time", "entity_stop_time",
                      "entity_cloud_type", "entity_len", "StSc_extent",
                      "Cu_extent", "Cu_center_time", "Cu_cores_nb",
                      "precip_extent", "StSc_thickness_min",
                      "StSc_thickness_max", "StSc_thickness_mean",
                      "StSc_thickness_std", "Other_occurance"]
    ...
    cloud_slices = ndimage.find_objects(np.asarray(labels_0, dtype=int))  # first part of slice height dim; second part time
    ...
    for cloud in tqdm(unique_labels[0:]):
        ...
        entity_start_time = entity_stop_time = entity_cloud_type =\
                      Cu_center_time =\
                      StSc_thickness_min = StSc_thickness_max =\
                      StSc_thickness_mean = StSc_thickness_std = np.nan
    ...
    #         continue
        ...
        cloud_slice_idx = cloud_slices[int(cloud)-1]  # cloud slices and labels differ by one index
        cloud_slice = np.where(labels_0[cloud_slice_idx] == 0, np.nan, labels_0[cloud_slice_idx])
        first_hgt_idx = cloud_slice_idx[0].start

        cbhs_idx = ccam.f_cloud_edges(np.where(np.isnan(cloud_slice), 0, 1))+first_hgt_idx
        ...
        if Cu_extent > 0:
            ...
            if Cu_cores_nb == 1:
                Cu_center_time = entity_start_idx+np.nanmean(Cu_idx).round(0).astype(int)

            # Precipitation
            precip_extent = get_precip(data, idx)

        if StSc_extent > 0:
            StSc_thickness_min, StSc_thickness_max, StSc_thickness_mean, StSc_thickness_std = get_stsc_macrophysics(StSc_idx, cbhs, cths)

        # Write results to array
        properties = [cloud, entity_start_time, entity_stop_time, entity_cloud_type,
                      entity_len, StSc_extent, Cu_extent, Cu_center_time, Cu_cores_nb,
                      precip_extent, StSc_thickness_min, StSc_thickness_max,
                      StSc_thickness_mean, StSc_thickness_std, ND_len]
    #     return properties
    ...
    cloud_data_merged['label'].attrs['description'] = 'Cloud entities created by applying a stencil '\
                            'to radar reflectiviy field greater than defined '\
                            'threshold'
    cloud_data_merged['label'].attrs['units'] = ''

    cloud_data_merged['CBH'] = (['range', 'time'], CBH)
    cloud_data_merged['CBH'].attrs['description'] = 'Cloud base height for each profile within a cloud entity.'
    cloud_data_merged['CTH'] = (['range', 'time'], CTH)
    cloud_data_merged['CTH'].attrs['description'] = 'Cloud top height for each profile within a cloud entity.'
    ...
    cloud_data_merged.attrs['institute'] = 'Max Planck Institute for Meteorology, Hamburg, Germany'
    cloud_data_merged.attrs['created_on'] = datetime.datetime.utcnow().strftime("%d/%m%/%Y %H:%M UTC")
    cloud_data_merged.attrs['created_with'] = os.path.basename(__file__) + " with its last modification on " + ctime(
                os.path.getmtime(os.path.realpath(__file__)))
    cloud_data_merged.attrs['version'] = git_module_version
    cloud_data_merged.attrs['environment'] = 'env:{}, numpy:{}'.format(sys.version, np.__version__)
    cloud_data_merged.attrs['source'] = '{} created with version {}'.format(filename_label, labels_netCDF.version)
    cloud_data_merged.attrs['Conventions'] = 'CF-1.7'

    # Write analysis output
    cloud_data_merged.to_netcdf(filename_entity,
                                encoding={
                                            "Zf": {'zlib': True},
                                            "label": {'zlib': True},
                                            "CTH": {'zlib': True},
